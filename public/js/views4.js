...
(function () {
  ...
  function renderPhases(el, ctx) {
    ...
    el.innerHTML =
      ...
      (canEdit ? (function () {
        const cityOpts = Object.keys((VS.CITY_COORDS) || {}).map(function (c) {
          var ll = VS.CITY_COORDS[c];
    ...
    // إحداثيات الموقع: منتقي مدينة يملأ الحقول + حفظ على المشروع
    const locCity = el.querySelector('#loc-city');
    if (locCity) locCity.addEventListener('change', function () {
      if (!this.value) return;
      const parts = this.value.split(',');
      el.querySelector('#loc-lat').value = parts[0];
      el.querySelector('#loc-lng').value = parts[1];
    });
    const locSave = el.querySelector('#loc-save');
    if (locSave) locSave.addEventListener('click', async function () {
      ...
      if (isNaN(lat) || isNaN(lng) || lat < 16 || lat > 33 || lng < 34 || lng > 56) {
        toast('أدخل إحداثيات صحيحة داخل المملكة (خط العرض 16–33، خط الطول 34–56) — أو اختر مدينة', true); return;
      }
      try {
  ...
  function renderPortfolio(el, ctx) {
    ...
    el.innerHTML =
      ...
      // خريطة مواقع كل المشاريع في المحفظة
      (VS.projectsMapHtml ? VS.projectsMapHtml(ctx) : '');
    ...
    if (VS.wireProjectsMap) VS.wireProjectsMap(el, ctx);
