...
DATA_DIR="$(pwd)/data"
PORT_FILE="$(pwd)/.port"
# تخزين الملفات المرفوعة (مستندات وصور) — مجلد مخصص خارج مجلد المشروع، منظّم كما طُلب:
# files/bassir-owner-eyes/{documents,images}/ — يُنشأ تلقائياً إن لم يوجد.
UPLOADS_HOST_DIR="${UPLOADS_HOST_DIR:-/root/files/bassir-owner-eyes}"
...
echo "==> [1/7] إيقاف وحذف حاوية bassir-owner-eyes الحالية فقط (إن وُجدت)..."
docker rm -f "${CONTAINER_NAME}" >/dev/null 2>&1 || true

echo "==> [2/7] حذف صورة bassir-owner-eyes القديمة فقط (إن وُجدت)..."
...
echo "==> [3/7] تحميل الصورة الجديدة من ${TAR_FILE}..."
...
echo "==> [6/7] تشغيل الحاوية الجديدة..."
docker run -d \
  --name "${CONTAINER_NAME}" \
  --restart unless-stopped \
  -p "${HOST_PORT}:3000" \
  -v "${DATA_DIR}:/app/data" \
  -v "${UPLOADS_HOST_DIR}:/app/uploads-external" \
  -e UPLOADS_DIR=/app/uploads-external \
  ${ENV_FLAG} \
  "${IMAGE_NAME}:${TAG}"
...
echo "   الملفات المرفوعة تُخزَّن في: ${UPLOADS_HOST_DIR}/{documents,images}"
